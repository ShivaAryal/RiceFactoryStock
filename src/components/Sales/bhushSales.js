...
class Bhush extends Component {
    ...
    onUpdate=()=>{
        const state = this.state;
        this.setState({loading:true})
        SalesService.editBhushSale(state.token,state.editId,state.editCustomer,state.editTotal/state.editPackets,state.editPackets,state.editDate).then(res=>{
    ...
    onDelete = (id) =>{
        this.setState({loading:true})
        SalesService.deleteBhushSale(this.state.token,id).then(res=>{
            this.setState({loading:false})
            this.refreshPage();
        }).catch(err=>{
            this.setState({loading:false})
        ...
        return (
          ...
          <Modal
                ...
                <ScrollView contentContainerStyle={styles.modalBackground} keyboardShouldPersistTaps="handled">
                    <View style={{backgroundColor:'#fff',padding:10,borderRadius:10}}>
                        <View style={{flexDirection:'row',justifyContent:'space-between'}}> 
                            <Text style={{fontSize:20,marginLeft:10,marginVertical:5}}>Edit Bhush Sales</Text>
                            {/* <MaterialIcons name="close" size={25} color="#000"  style={{alignSelf:'center'}}
                                onPress={()=>this.setState({visible:false})}
                            /> */}
                            <FAB
