...
class Bran extends Component {
    ...
    onSubmit=()=>{
        ...
        Keyboard.dismiss();
        const state = this.state;
        SalesService.postBranSales(state.token,state.customer,state.totalAmount/state.noofPackets,state.noofPackets,state.date).then(res=>{
    ...
    onUpdate=()=>{
        const state = this.state;
        this.setState({loading:true})
        SalesService.editBranSale(state.token,state.editId,state.editCustomer,state.editTotal/state.editPackets,state.editPackets,state.editDate).then(res=>{
    ...
    onDelete = (id) =>{
        this.setState({loading:true})
        SalesService.deleteBranSale(this.state.token,id).then(res=>{
            this.setState({loading:false})
            this.refreshPage();
        }).catch(err=>{
            this.setState({loading:false})
        ...
        return (
          ...
          <Modal
                ...
                <ScrollView contentContainerStyle={styles.modalBackground} keyboardShouldPersistTaps="handled">
                    <View style={{backgroundColor:'#fff',padding:10,borderRadius:10}}>
                        <View style={{flexDirection:'row',justifyContent:'space-between'}}> 
                            <Text style={{fontSize:20,marginLeft:10,marginVertical:5}}>Edit Bran Sales</Text>
                            {/* <MaterialIcons name="close" size={25} color="#000"  style={{alignSelf:'center'}}
                                onPress={()=>this.setState({visible:false})}
                            /> */}
                            <FAB
